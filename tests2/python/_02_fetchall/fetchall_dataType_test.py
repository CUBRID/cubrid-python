...
#               test normal string type
                ...
                try:
                   rowNum = self.cur.execute(sqlInsert)
                    ...
                    self.assertEquals(errorValue,"(-494, \"ERROR: DBMS, -494, Semantic: Cannot coerce \'a\' to type nchar. insert into character_db (c_nchar) values ( cast(\'a\' as nchar(4))), ( cast(\'abcd\' as nchar(4))), ( cast(\'abcdefg\' as nchar(4)))\")");
                sqlSelect = "select * from character_db"
                self.cur.execute(sqlSelect)
		results=self.cur.fetchall()
		for res in results:
