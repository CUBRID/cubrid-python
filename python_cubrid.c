                       ...
                       PyObject * kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords (args, kwargs,
    ...
    for (i = 0; i < len; i++) {
        if (unescape_string[i] == '\'') {
            escape_str_len++;
        }
        escape_str_len++;
    }

    if (escape_str_len == len) {
#if PY_MAJOR_VERSION >= 3
        op = PyUnicode_FromStringAndSize (unescape_string, len);
#else
        op = PyString_FromStringAndSize (unescape_string, len);
#endif
        return op;
    }
    ...
    escape_string[escape_str_len] = '\0';

#if PY_MAJOR_VERSION >= 3
    op = PyUnicode_FromStringAndSize (escape_string, escape_str_len);
#else
    op = PyString_FromStringAndSize (escape_string, escape_str_len);
#endif
...
#define _CUBRID_VERSION_	"9.0.0.0001"
