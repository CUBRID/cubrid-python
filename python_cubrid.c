...
#define gmtime(t) _gmtime32(t)

#if ( _MSC_VER >= 1500 )	/* if VS version is 2008 or higher  */
#define localtime(t) _localtime32(t)
...
#define gmtime_r(t, tm) \
    (gmtime(t)?memcpy((tm), gmtime(t), sizeof(*(tm))):NULL)
#define localtime_r(t, tm) \
    (localtime(t)?memcpy((tm), localtime(t), sizeof(*(tm))):NULL)
#endif
...
static char _cubrid_Date__doc__[] = 
"Date(year, month, day) --\n\
This function constructs an object holding a date value.\n\
\n\
Parameters::\n\
  year: int, supported range is 1-9999\n\
  month: int, supported range is 1-12\n\
  day: int, supported range is 1-31\n\
\n\
Return a date value string.\n\
\n\
Example::\n\
  dt = _cubrid.Date(2011,6,1)\n\
  cur.prepare(\"insert into test_cubrid(birthday) values (?)\")\n\
  cur.bind_param(1, dt)\n\
  cur.execute()";

static PyObject *
_cubrid_Date (PyObject * self, PyObject * args)
{
  int year, month, day;
  char buf[16] = { '\0' };
...
static char _cubrid_Time__doc__[] = 
"Time(hour, minute, second)\n\
This function constructs an object holding a time value.\n\
\n\
Parameters::\n\
  hour: int, supported range is 0-23\n\
  minute: int, supported range is 0-59\n\
  second: int, supported range is 0-59\n\
\n\
Return a time value string\n\
\n\
Example::\n\
  tm = _cubrid.Time(11, 29, 59)\n\
  cur.prepare('insert into test_cubrid(lunch) values (?)')\n\
  cur.bind_param(1, tm)\n\
  cur.execute()";

static PyObject *
_cubrid_Time (PyObject * self, PyObject * args)
{
  ...
  snprintf (buf, 16, "%d:%d:%d", hour, minute, second);

  return _cubrid_return_PyString_FromString (buf);
}

static char _cubrid_Timestamp__doc__[] =
...
Parameters::\n\
  ...
  second: int, supported range is 0-59\n\
\n\
Return a timestamp value string.\n\
\n\
Example::\n\
  ts = _cubrid.Timestamp(2011, 6, 1, 11, 29, 59)\n\
  cur.prepare('insert into test_cubrid(lunch) values (?)')\n\
  cur.bind_param(1, ts)\n\
  cur.execute()";

static PyObject *
_cubrid_Timestamp (PyObject * self, PyObject * args)
{
  int year, month, day, hour, minute, second;
  char buf[32] = { '\0' };

  if (!PyArg_ParseTuple
      (args, "iiiiii", &year, &month, &day, &hour, &minute, &second))
    {
      return NULL;
    }

  if (year <= 0 || year > 9999 || month <= 0 || month > 12 || day <= 0
      || day > 31 || hour < 0 || hour > 24 || minute < 0 || minute > 59
      || second < 0 || second > 59)
    {
      return handle_error (CUBRID_ER_INVALID_PARAM, NULL);
    }

  snprintf (buf, 32, "%d-%d-%d %d:%d:%d", year, month, day, hour, minute,
	    second);

  return _cubrid_return_PyString_FromString (buf);
}

static char _cubrid_DateFromTicks__doc__[] = 
"DateFromTicks(ticks)\n\
...
static PyObject *
_cubrid_DateFromTicks (PyObject * self, PyObject * args)
{
  double ticks;
  struct tm tm;
  time_t t;
  ...
  if (localtime_r (&t, &tm))
    {
      args =
	Py_BuildValue ("iii", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
      if (args)
	{
	  result = _cubrid_Date (self, args);
	  Py_DECREF (args);
	}
    }
  return result;
...
static char _cubrid_TimeFromTicks__doc__[] = 
"TimeFromTicks(ticks)\n\
...
static PyObject *
_cubrid_TimeFromTicks (PyObject * self, PyObject * args)
{
  double ticks;
  struct tm tm;
  time_t t;
  ...
  if (localtime_r (&t, &tm))
    {
      args = Py_BuildValue ("iii", tm.tm_hour, tm.tm_min, tm.tm_sec);
      if (args)
	{
	  result = _cubrid_Time (self, args);
	  Py_DECREF (args);
	}
    }
  return result;
...
static char _cubrid_TimestampFromTicks__doc__[] =
...
static PyObject *
_cubrid_TimestampFromTicks (PyObject * self, PyObject * args)
{
  double ticks;
  struct tm tm;
  time_t t;
  ...
  if (localtime_r (&t, &tm))
    {
      args =
	Py_BuildValue ("iiiiii", tm.tm_year + 1900, tm.tm_mon + 1,
		       tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
      if (args)
	{
	  result = _cubrid_Timestamp (self, args);
	  Py_DECREF (args);
	}
    }
  return result;
...
static struct PyMethodDef _cubrid_methods[] = {
  ...
  {
   "Date",
   (PyCFunction) _cubrid_Date,
   METH_VARARGS,
   _cubrid_Date__doc__},
  {
   "Time",
   (PyCFunction) _cubrid_Time,
   METH_VARARGS,
   _cubrid_Time__doc__},
  {
   "Timestamp",
   (PyCFunction) _cubrid_Timestamp,
   METH_VARARGS,
   _cubrid_Timestamp__doc__},
  {
   "DateFromTicks",
   (PyCFunction) _cubrid_DateFromTicks,
   METH_VARARGS,
   _cubrid_DateFromTicks__doc__},
  {
   "TimeFromTicks",
   (PyCFunction) _cubrid_TimeFromTicks,
   METH_VARARGS,
   _cubrid_TimeFromTicks__doc__},
  {
   "TimestampFromTicks",
   (PyCFunction) _cubrid_TimestampFromTicks,
   METH_VARARGS,
   _cubrid_TimestampFromTicks__doc__},
  {NULL, NULL}
};
...
#define _CUBRID_VERSION_	"8.4.0.1.0.0"
